...
import { BaseMapDetailComponent } from './detail/basemap-detail.component';
...
export class BaseMapComponent implements OnInit, OnDestroy {
  ...
  //Business variables
  public items: BaseMapDto[];
  public selectedItems: BaseMapDto[] = [];
  actionItem: BaseMapDto;
  ...
  getPermission() {
    this.hasPermissionUpdate = this.permissionService.getGrantedPolicy('BaseMaps.Edit');
    this.hasPermissionDelete = this.permissionService.getGrantedPolicy('BaseMaps.Delete');
  ...
  loadData() {
    ...
    this.BaseMapService
      ...
      .subscribe({
        next: (response: PagedResultDto<BaseMapDto>) => {
  ...
  showAddModal() {
    const ref = this.dialogService.open(BaseMapDetailComponent, {
      header: 'Thêm bản đồ nền',
      width: DIALOG_MD,
    });

    ref.onClose.pipe(takeUntil(this.ngUnsubscribe)).subscribe((data: BaseMapDto) => {
      if (data) {
