...
const iconDenounceLand = new L.Icon({
  iconUrl: 'assets/images/map/marker-denounce-land.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconComplainEnv = new L.Icon({
  iconUrl: 'assets/images/map/marker-complain-env.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconDenounceEnv = new L.Icon({
  iconUrl: 'assets/images/map/marker-denounce-env.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconComplainMin = new L.Icon({
  iconUrl: 'assets/images/map/marker-complain-min.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconDenounceMin = new L.Icon({
  iconUrl: 'assets/images/map/marker-denounce-min.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconComplainWater = new L.Icon({
  iconUrl: 'assets/images/map/marker-complain-water.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
const iconDenounceWater = new L.Icon({
  iconUrl: 'assets/images/map/marker-denounce-water.png',
  iconSize: [25, 41],
  iconAnchor: [12, 41],
  popupAnchor: [1, -34],
  shadowSize: [41, 41],
...
export class MapComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private getSysConfigAndInitMap() {
    ...
    if (storedConfig) {
      ...
      if (currentTime < expiry) {
        ...
        let centerStr = configs.find(x => x.name === SysConfigConsts.MAP_CENTER)?.value;
        this.center = this.convertStringCoordiate(centerStr);
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    /*
    if (!changes.basemap?.isFirstChange()) {
  ...
  initMap() {
    ...
    this.info.buildInfoContent = function (props: any, type: any) {
      var infoContent =
        ...
        <tr>\
          <th scope="row">\
          <td>Bộ phận thực hiện:</td><td colspan=3>' +
            props.boPhanDangXL +
          '</td>\
        </tr>\
        <tr>\
    ...
    legend.onAdd = function (map) {
      var div = L.DomUtil.create('div', 'leaflet-control-info legend'),
        types = [1, 2, 3, 4],
        labels = ['Đất đai', 'Môi trường', 'Khoáng sản', 'Tài nguyên nước'],
        bgColors = ['#D32F2F', '#388E3C', '#F57C00', '#42A5F5'];

      // loop through our density intervals and generate a label with a colored square for each interval
      for (var i = 0; i < types.length; i++) {
        div.innerHTML += '<i style="background:' + bgColors[i] + '"></i> ' + labels[i] + '<br>';
      }
  ...
  letDraw() {
    ...
    this.map.on('draw:created', e => {
      ...
      if (type != 'marker' && type != 'circlemarker') {
        ...
        var point: any;
        if (type == 'circle') point = layer.getLatLng();
        else {
    ...
    data
      ...
      .forEach(dataMap => {
        ...
        const marker = L.marker(toado, {
          icon: dataMap.loaiVuViec === LoaiVuViec.KhieuNai ? 
                (dataMap.linhVuc === LinhVuc.DatDai ? iconComplainLand : 
                  dataMap.linhVuc === LinhVuc.MoiTruong ? iconComplainEnv : 
                  dataMap.linhVuc === LinhVuc.KhoangSan ? iconComplainMin : 
                  iconComplainWater)
                 : 
                (dataMap.linhVuc === LinhVuc.DatDai ? iconDenounceLand : 
                  dataMap.linhVuc === LinhVuc.MoiTruong ? iconDenounceEnv :
                  dataMap.linhVuc === LinhVuc.KhoangSan ? iconDenounceMin :
                   iconDenounceWater),
        });
        ...
        if (dataMap.duLieuHinhHoc) {
          ...
          //  geometry = JSON.parse(dataMap.duLieuHinhHoc);
          if (geojson == undefined) geojson = JSON.parse(dataMap.duLieuHinhHoc);
          ...
          */
          ...
          if (geojson.geometry.type != 'Point') {
            ...
            if (dataMap.loaiVuViec == LoaiVuViec.KhieuNai) newlayer.addTo(this.khieunai);
            else if (dataMap.loaiVuViec == LoaiVuViec.ToCao) newlayer.addTo(this.tocao);
  ...
  renderBaseMap() {
    //1. Hiện: Có tên trong ds basemap -> 1.1. Đã load, 1.2. chưa load
    //2. Ẩn: 0 có tên trong ds basemap -> nếu đã load rồi -> ẩn đi
    if (this.basemap!=undefined){
      ...
      for (let i = 0; i < this.basemap.length; i++) {
        var loaded = false;
        var index = -1;
        for (let j = 0; j < this.basemapLoaded.length; j++) {
          if (this.basemap[i] == this.basemapLoaded[j].name) {
            loaded = true;
            index = j;
            break; //đã load
          }           
        }
        if (!loaded){//Nếu chưa load thì load từ Geoserver
          var bandonen = L.tileLayer.wms(`${this.geoserverUrl}/geoserver/kntc/wms?`, {
            layers: 'kntc:' + this.basemap[i],
            format: 'image/png',
            transparent: true,
          });
  
          this.quyhoach.addLayer(bandonen);

          bandonen.on('load', () => {
          });

          bandonen.on('tileerror', error => {
          ...
          this.basemapLoaded.push(obj); 
        }  
        else//load rồi thì chỉ hiển thị lên thôi
        {//Chỉ hiển thị lên nếu chưa hiển thị
          if (!this.quyhoach.hasLayer(this.basemapLoaded[index].data))
            this.quyhoach.addLayer(this.basemapLoaded[index].data);
        } 
        this.quyhoach.addTo(this.map);
      }
        
      //Ẩn      
      for (let i = 0; i < this.basemapLoaded.length; i++) {//Ktra ds đã load
        var removed = true; 
        for (let j = 0; j < this.basemap.length; j++){
          if (this.basemapLoaded[i].name == this.basemap[j]) {
            removed = false;
            break; //0 phải remove
          }
        }
        if (removed) {//Đã load rồi mà 0 có tên ở đây
          this.quyhoach.removeLayer(this.basemapLoaded[i].data);
        }
      }
